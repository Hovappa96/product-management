...
const register = async function (req, res) {
    try {
        ...
        encrypted = await bcrypt.hash(data.password, salt);

        const isAddress = JSON.parse(address)
        const { shipping, billing } = isAddress
...
const login = async function (req, res) {
    try {
        ...
        const token = jwt.sign({
            ...
            group: "11",    //1 hours:minute:second
        }, "group11", { expiresIn: "30m" })
